...
import org.jmlspecs.openjml.jmltest.JmlTestDS;
...
public class JmlAssertionAdder extends JmlTreeScanner {
    ...
    /** Method added for JmlTest
     * @author soham
     */
    
    public JmlAssertionAdder(Context context, boolean jmltest) {
...
//          n.flags = that.flags;
    ...
    public void visitImport(JCImport that, boolean jmltest) {
        visitImport(that);
    }
...
//        addStat(M.at(p).If(e, st, null));
    ...
    public void visitJmlMethodDecl(JmlMethodDecl that, boolean jmltest) {
        ...
        try {
            ...
            jmltestdsObj.setBody(that.body);
...
//        }
        ...
        try {
            ...
            {
            JmlTestDS jmltestdsObj = new JmlTestDS();   //move this outside!!!!!
            ...
            JmlMethodSpecs cases;
                   
            body = that.body;
            modifier = that.mods;
            rettype = that.restype;     //def
            parameters = that.params;   //def/use
            ...
            if (cases != null) {
                ...
                if(parameters.length() > 0) {
                    ...
                    for (JCVariableDecl param : parameters) {
                        int linecount = 0; //required to find line number on the body of the code whiloib n
                        
                        String paramname = param.name.toString();
                        boolean paramflag = false;
                        
                        if (body.toString().contains(paramname)) {
                            paramflag = true;
                            Iterator<JCStatement> iter = null;
                            
                            iter = body.stats.iterator();
                            if (iter.hasNext()) {
                                JCStatement st = body.stats.get(linecount);
                                /*if (st.toString().contains("=")) {
                                    
                                    String[] classify = st.toString().split("=");
                                    int side = 0;
